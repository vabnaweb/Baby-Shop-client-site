...
const ManageAllOrdersData = (props) => {
    const {_id, name, email, address, age, Price, status} = props.orderall;
    const [user, setUser] = useState({})

    const handleDelete = (id) => {
        const proceed = window.confirm("Are you sure? Do you want to cancel this Order?")
        if (proceed) {
            const url = `https://dry-mountain-44007.herokuapp.com/orders/${id}`;
            fetch(url, {
                method: "DELETE"
            })
                .then(res => res.json())
    ...
    const hanldeApproved = (e) => {
        const url = `https://dry-mountain-44007.herokuapp.com/orders/${_id}`;
        fetch(url, {
            method: "PUT",
            headers: {
